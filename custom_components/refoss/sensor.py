...
class EnergyFileWatcher(FileSystemEventHandler):
    """Watch for changes in the energy JSON file and reload data."""

    def __init__(self, file_path: str, sensor_instance):
    ...
    def on_modified(self, event):
        ...
        if event.src_path == self._file_path:
            _LOGGER.info("Detected change in %s, reloading energy data.", self._file_path)
...
class RefossSensor(RefossEntity, SensorEntity):
    ...
    def __init__(
        ...
        self.file_path = f"/config/{self.coordinator.device.dev_name}_monthly_energy.json"
    ...
    def ensure_file_exists(self):
        """Ensure the JSON file exists, creating it with default values if necessary."""
        if not os.path.exists(self.file_path):
            try:
                # ✅ 현재 기기의 실제 채널 개수를 기반으로 기본값 설정
                default_data = {str(channel): 0 for channel in self.coordinator.device.channels}
                with open(self.file_path, "w", encoding="utf-8") as file:
                    json.dump(default_data, file, indent=4)
                _LOGGER.info("Created new energy data file: %s", self.file_path)
            except IOError as e:
    ...
    def load_energy_data(self):
        ...
        try:
            ...
            _LOGGER.info("Loaded stored energy data from %s", self.file_path)
    ...
    def start_watching_file(self):
        """Start watching the energy JSON file for changes."""
        event_handler = EnergyFileWatcher(self.file_path, self)
        observer = Observer()
        observer.schedule(event_handler, os.path.dirname(self.file_path), recursive=False)
        observer.start()
        _LOGGER.info("Started watching %s for changes.", self.file_path)
